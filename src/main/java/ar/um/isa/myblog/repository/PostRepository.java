package ar.um.isa.myblog.repository;
...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(
        value = "select post from Post post left join fetch post.blog left join fetch post.tag",
        countQuery = "select count(post) from Post post"
    )
    Page<Post> findAllWithToOneRelationships(Pageable pageable);

    @Query("select post from Post post left join fetch post.blog left join fetch post.tag")
    List<Post> findAllWithToOneRelationships();

    @Query("select post from Post post left join fetch post.blog left join fetch post.tag where post.id =:id")
